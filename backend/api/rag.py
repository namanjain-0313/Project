...
def answer_question(question: str) -> dict:
    ...
    4. Send to Claude with context → get intelligence brief
    ...
    if similar_events:
        ...
        for i, event in enumerate(similar_events[:10], 1):
            context_parts.append(
                f"{i}. [{event['date']}] {event['actor1']} {event['event_type']} {event['actor2']}\n"
                f"   Headline: {event['headline']}\n"
                f"   Hostility: {event['goldstein']} | Source: {event['source_url']}"
            )
    ...
    # ── Step 4: Claude generates the intelligence brief ────
...
information, say so clearly. Do not add information from your training data."""
    ...
    try:
        ...
        sources = list(set(
            e['source_url'] for e in similar_events
